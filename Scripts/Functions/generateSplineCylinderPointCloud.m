function [pointCloud, imageMatrix] = generateSplineCylinderPointCloud(spline_definition_points, cylinder_radius, num_cloud_points, use_smooth_spline_flag, patchColor, markerColor, output_filename_base, alpha_mod, createSurface, createSTL)
...
%   markerColor              - 1x3 RGB color for the scatter points [R G B].
...
%% --- Input Validation ---
if nargin ~= 8
    error('Function requires 8 input arguments.');
end
...
scatter3(ax1, pointCloud(:,1), pointCloud(:,2), pointCloud(:,3), 5, 'filled', 'MarkerFaceColor', markerColor);
hold(ax1, 'off');
...
scatter3(ax2, pointCloud(:,1), pointCloud(:,2), pointCloud(:,3), 5, 'filled', 'MarkerFaceColor', markerColor);
hold(ax2, 'off');
...
scatter3(ax3, pointCloud(:,1), pointCloud(:,2), pointCloud(:,3), 5, 'filled', 'MarkerFaceColor', markerColor);
hold(ax3, 'off');
...
scatter3(ax4, pointCloud(:,1), pointCloud(:,2), pointCloud(:,3), 5, 'filled', 'MarkerFaceColor', markerColor);
hold(ax4, 'off');
...
%              'Color', 'white', ...
%              'Units', 'pixels', ...
%              'Position', [100, 100, 1000, 1000]); % Set larger size
% ax = axes(fig); 
% hold(ax, 'on');
% 
% if ~isempty(TR) % Use the triangulation object for visualization too
%     try
%         trisurf(TR, 'Parent', ax, 'FaceColor', patchColor, 'FaceAlpha', 0.5, 'EdgeColor', 'none');
%     catch trisurf_err
%         warning('Failed to display triangulation surface: %s', trisurf_err.message);
%     end
% else
%     fprintf(' Triangulation is empty, only plotting points.\n');
% end
% scatter3(ax, pointCloud(:,1), pointCloud(:,2), pointCloud(:,3), 15, 'filled', 'MarkerFaceColor', markerColor);
% axis(ax, 'equal'); axis(ax, 'off'); view(ax, 45, 35.26); grid(ax, 'off'); hold(ax, 'off');
% 
% %% --- Capture the figure as an image matrix ---
% fprintf('Capturing figure frame...\n'); drawnow; frame = getframe(fig); imageMatrix = frame.cdata; fprintf('Frame captured.\n'); close(fig);
