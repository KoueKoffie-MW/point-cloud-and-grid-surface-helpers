...
% modern arguments block with validation attributes.
    ...
    %% --- Write STL if requested ---
    if createSTL && ~isempty(TR)
        try
            outputDir = fileparts(output_filename_base);
            ...
            warning('Failed to write STL file: %s', stl_err.message);
        end
    end

    %% --- Visualization ---
    if nargout > 1 || true % always generate image for mask
