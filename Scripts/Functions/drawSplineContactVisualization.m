...
% modern arguments block with validation attributes.
    ...
    diskColor  = [0.8500 0.3250 0.0980];
    titleFontSize = 28;
...
function centers = computeDiskCentersAlongSpline(points, L, numDisks)
    ...
    for k = 2:numDisks
        ...
        while idx < size(points,1) && currentDist < targetDist
            idx = idx + 1;
            currentDist = currentDist + norm(points(idx,:) - points(idx-1,:));
        end
...
function drawOrientedDisks3D(ax, centers, R, color, points)
    for i = 1:size(centers,1)
        ...
        verts = c + R * (n1 .* cos(theta)' + n2 .* sin(theta)');
...
function drawSingleCylinder3D(ax, points, R, L, color)
    if L <= 0 || isempty(points), return; end
    targetDist = L / 2;
    currentDist = 0;
    idx = 1;
    while idx < size(points,1) && currentDist < targetDist
